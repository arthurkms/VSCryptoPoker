...
#funcao encrypt para ElGamal comutativo
#the encrypt version for commutative ElGamal
def encrypt2(key, encrypted_text, n_order):
    # normal case
    if n_order == 1:
        return encrypt(key, encrypted_text)

    z = encode(encrypted_text, key.iNumBits)

    cipherArray = encrypted_text.split()
    # print "tamanho cipherArray ", len(cipherArray)

    cipher_pairs = []
    # goes through the n
    for n in range(0, len(cipherArray), n_order):
        temp_list = []
        for i in range(0, n_order - 1):
            print "iteracoes de i ", i
            temp_list.append(cipherArray[n + i])

        y = random.randint(0, key.p)
        c = modexp(key.g, y, key.p)
        temp_list.append(c)

        d_old = int(cipherArray[n + n_order - 1])

        d_new = (d_old * modexp(key.h, y, key.p)) % key.p
        temp_list.append(d_new)

        cipher_pairs.append(temp_list)

    encryptedStr = ""
    for pair in cipher_pairs:
        for n in range(0, len(pair)):
            encryptedStr += str(pair[n]) + ' '
...
#funcao decrypt para ElGamal comutativo
#The decrypt version for commutative ElGamal
def decrypt2(key, cipher, n_order, n_total):
    # decrpyts each pair and adds the decrypted integer to list of plaintext inte    gers
    ...
    semi_encrypted_text = []
    # print "tamanho cipherArray ", len(cipherArray)
    # print "tamanho divisao ",n_total-n_order+2

    for i in range(0, len(cipherArray), n_total - n_order + 2):
        semi_encrypted_line = []
        for n in range(0, n_total - n_order):
            semi_encrypted_line.append(cipherArray[n + 1])

        # c = first number in pair
        c = int(cipherArray[i + n_total - n_order - 1])
        # d = second number in pair
        d = int(cipherArray[i + n_total - n_order + 1])
        ...
        plain = (d * modexp(s, key.p - 2, key.p)) % key.p
        # add plain to list of plaintext integers
        semi_encrypted_line.append(plain)

        semi_encrypted_text.append(semi_encrypted_line)

    semi_encryptedStr = ""
    for line in semi_encrypted_text:
        for n in range(0, len(line)):
            semi_encryptedStr += str(line[n]) + ' '

    print "semi-texto", semi_encryptedStr

    return semi_encryptedStr
